...
void service_toggleGPIO_on_input_change() {
    static int last_input_state = -1;
    static int output_state = 0;

    int input_state = readGPIOValue(GPIO27_INPUT);
    if (input_state == -1) return;

    std::cout << "[Input GPIO] Value: " << input_state << std::endl;
...
int main() {
    ...
    sequencer.addService(service_toggleGPIO_on_input_change, 1, 98, 100);
